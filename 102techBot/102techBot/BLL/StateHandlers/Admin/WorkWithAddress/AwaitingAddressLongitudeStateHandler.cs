...
{
    /// <summary>
    /// Обработка состояния ожидания ввода долготы при добавлении нового адреса в систему
    /// </summary>
    internal class AwaitingAddressLongitudeStateHandler : BaseStateHandler
    {
        private readonly UserService _userService;
        public AwaitingAddressLongitudeStateHandler(
            IUI ui,
        ...
        public override async Task HandleStateAsync(User user, MessageDTO message)
        {
            if (!string.IsNullOrEmpty(message.Text))
            {
                ...
                newAddress!.Longitude = double.Parse(message.Text);
                _storageRepository.Add(user.Id.ToString(), newAddress);

                await _UI.SendMessageAsync(user.Id, $"Телефон:");
                await _userService.ChangeStateAsync(user, UserState.AwaitingAddressPhone);
            }
            else
            {
